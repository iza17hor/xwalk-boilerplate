...
async function customizeButtons(sk) {
  const container = (await getElement(sk, '.plugin-container')) as HTMLElement;
  container.style.visibility = 'hidden';

  // hide the default buttons once
  container.querySelectorAll<HTMLElement>('.plugin').forEach((plugin) => {
  ...
  new MutationObserver((mutations) => {
    mutations.forEach((mutation) =>
      mutation.addedNodes.forEach((node) => {
        ...
        if (shouldHidePlugin(currentNode)) {
